...
def update(i):
    ...
    line = [(x[i]+xx, buffer[i]) for i in range(len(buffer))]
    draw.DrawLines(line, color='green1')
...
window = sg.Window('Fourier', layout)
draw = window['Graph']

i = 0
timeout = delta = 10    # Target refresh interval = 10 milliseconds
